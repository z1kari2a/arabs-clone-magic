...
import type { PurchaseOrder, Supplier, PriceTier } from "./erp-types";
import { computePO, cartonsOf } from "./erp-store";
...
export function buildPurchaseOrderHTML(o: PrintOptions): string {
  ...
  const rows = po.rows
    .map((r, i) => ({ r, i }))
    .filter(({ r }) => r.model || r.name || (r.qty || 0) > 0);
  ...
  const distLabel =
    po.distributionType === "cbm" ? "حسب تكلفة CBM"
    : po.distributionType === "percentage" ? "حسب التكلفة المئوية"
    : "حسب متوسط التكلفة";
  ...
  const itemRows = rows
    .map(({ r, i }, idx) => {
      ...
      return `<tr>
        ...
        <td class="c">${esc(r.currency ?? po.currency)}</td>
  ...
  <table class="grid">
    ...
    <tbody>${itemRows || `<tr><td colspan="17" class="c empty">لا توجد بنود</td></tr>`}</tbody>
    <tfoot>
      <tr>
        <td colspan="5" class="r">الإجمالي</td>
        <td class="l">${auto(m.totalQty)}</td>
        <td colspan="2" class="l">${auto(m.totalInvoiceAmount)} ${esc(po.currency)}</td>
  ...
  const tiersTable =
    priceTiers.length && rows.length
      ? `<div class="sec">التسعيرات حسب الوجهات (USD)</div>
         <table class="grid">
           <thead><tr>
