...
function SuppliersPage() {
  ...
  const filtered = list.filter(
    (s) =>
      !search || s.name.includes(search) || s.code.includes(search) || s.country.includes(search),
  );
  const patch = (i: number, p: Partial<Supplier>) =>
    setList(list.map((s, idx) => (idx === i ? { ...s, ...p } : s)));
  ...
  const onNew = () => {
    ...
    setList([
      ...list,
      {
        code: `SUP${String(max + 1).padStart(4, "0")}`,
        name: "",
        country: "",
        city: "",
        phone: "",
  ...
  const onSave = (): boolean => {
    ...
    for (const s of list) {
      if (!s.code.trim()) {
        toast.error("يوجد مورد بدون كود");
        return false;
      }
      if (codes.has(s.code)) {
        toast.error(`كود مكرر: ${s.code}`);
        return false;
      }
      codes.add(s.code);
    }
  ...
  const onFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (const row of data) {
      ...
      const active = ["نشط", "true", "1", "yes", "active"].includes(activeText)
        ? true
        : ["موقوف", "false", "0", "no", "inactive"].includes(activeText)
          ? false
          : undefined;
      ...
      if (at < 0) {
        ...
        next[at] = {
          ...prev,
          name: text(name, prev.name),
          country: text(country, prev.country),
          city: text(city, prev.city),
          phone: text(phone, prev.phone),
          email: text(email, prev.email),
          currency: text(currency, prev.currency),
          notes: text(notes, prev.notes),
    ...
    if (!added && !updated)
      return toast.error("لا يوجد عمود «كود المورد» في الملف — لم يُستورد شيء");
    setList(next);
    setEditing(true);
    toast.success(
  ...
  const actions = [
    ...
    },
    {
      ...
      label: "حذف",
      color: "text-rose-600",
      onClick: () => toast.info("استخدم زر الحذف في نهاية سطر المورد"),
    },
    {
      icon: Search,
      label: "بحث",
      color: "text-indigo-500",
      onClick: () => document.getElementById("sup-search")?.focus(),
    },
      ...
      <div className="bg-white border border-slate-300 rounded">
        ...
        >
          <div className="flex items-center gap-2 font-semibold text-slate-700">
            <Building2 size={16} /> قائمة الموردين ({filtered.length})
          </div>
          <input
            id="sup-search"
            placeholder="بحث..."
            ...
            className="px-2 py-1 text-xs border border-slate-300 rounded w-56 text-right"
          />
        </div>
        <ErpTable
          headers={[
          ...
          {filtered.map((s, i) => {
            ...
            return (
              <tr key={s.code + i} className="hover:bg-blue-50/40">
                ...
                <Cell
                  value={s.country}
                  onChange={(v) => patch(idx, { country: v })}
                  disabled={!canEditRow}
                  align="right"
                />
                <Cell
                  value={s.city}
                  onChange={(v) => patch(idx, { city: v })}
                  disabled={!canEditRow}
                  align="right"
                />
                <Cell
                  value={s.phone}
                  onChange={(v) => patch(idx, { phone: v })}
                  disabled={!canEditRow}
                />
                <Cell
                  value={s.email}
                ...
                <Cell
                  value={s.currency}
                  onChange={(v) => patch(idx, { currency: v })}
                  disabled={!canEditRow}
                />
                ...
                <td className="border border-slate-200 text-center">
                  <button
                    disabled={!canEditRow}
                    onClick={() => patch(idx, { active: !s.active })}
                    className={`px-2 py-0.5 rounded text-xs ${s.active ? "bg-emerald-100 text-emerald-700" : "bg-slate-200 text-slate-600"} disabled:opacity-70`}
                  >
                    {s.active ? "نشط" : "موقوف"}
                  </button>
                </td>
                <td className="border border-slate-200 text-center">
                  <button
                    ...
                    disabled={!mayDelete}
                    title={mayDelete ? "حذف المورد" : "الحذف يتطلب صلاحية مدير"}
