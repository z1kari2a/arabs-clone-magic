...
function ExpensesPage() {
  ...
  const filtered = useMemo(() => {
    return allExpenses.filter(({ po, e, supplierName }) => {
      ...
      if (q) {
        ...
        if (!`${e.type} ${e.note} ${po.number} ${supplierName}`.toLowerCase().includes(s))
          return false;
      }
      return true;
    });
  ...
  const stats = useMemo(() => {
    ...
    const byOrder = new Map<
      string,
      { number: string; supplier: string; amount: number; date: string }
    >();
    filtered.forEach((r) => {
  ...
  const actions = [
    ...
    },
    {
      ...
      onClick: () => {
        if (!orders.length) {
          toast.error("لا توجد أوامر شراء - أنشئ أمر شراء أولاً");
          return;
        }
        setTargetPO(orders[0].number);
        setExpDlg(true);
      },
    },
    {
      ...
      label: "تصفية",
      color: "text-blue-600",
      onClick: () => document.getElementById("exp-filter-q")?.focus(),
    },
    {
      icon: Search,
      label: "بحث",
  ...
  return (
    <ErpLayout title="المصروفات" ribbon={<Ribbon actions={actions} />}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-2">
        ...
        <StatCard
          label="عدد الأوامر"
          value={fmtInt(stats.ordersCount)}
          icon={Package}
          tone="bg-indigo-50 text-indigo-700 border-indigo-200"
        />
        <StatCard
          label="متوسط/أمر شراء"
        ...
        <StatCard
          label="متوسط/بند"
          value={fmt(stats.avgPerLine)}
          icon={DollarSign}
          tone="bg-teal-50 text-teal-700 border-teal-200"
        />
        <StatCard
          label="سعر CBM"
          value={fmt(stats.cbmCost, 2)}
          icon={BarChart3}
      ...
      <div className="bg-white border border-slate-300 rounded p-2 grid grid-cols-1 md:grid-cols-6 gap-2 items-end">
        <FilterField label="بحث">
          <input
            ...
            placeholder="نوع/بيان/رقم أمر/مورد"
        ...
        <ChartPanel title="المصروفات حسب النوع" icon={PieIcon}>
          {stats.byTypeArr.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={240}>
              <PieChart>
                <Pie
                  data={stats.byTypeArr}
                  dataKey="value"
                  nameKey="name"
                  outerRadius={80}
                  label={(d) => d.name}
                >
                  {stats.byTypeArr.map((_, i) => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-2">
        <div className="lg:col-span-2 bg-white border border-slate-300 rounded">
          <div
            className="flex items-center gap-2 px-3 py-1.5 border-b border-slate-300"
            style={{ background: "var(--color-erp-panel-header)" }}
          >
            <Wallet size={14} />
            <span className="font-semibold text-slate-700 text-xs">
              تفاصيل بنود المصروفات ({fmtInt(filtered.length)})
            </span>
          </div>
          ...
          {stats.byCurrencyArr.length > 0 && (
            <div className="border-t border-slate-200 p-2">
              <div className="text-[11px] font-semibold text-slate-600 mb-1">
                الإجماليات حسب العملة الأصلية
              </div>
              <div className="space-y-1">
                {stats.byCurrencyArr.map((c) => (
                  <div
                    key={c.name}
                    className="flex items-center justify-between text-xs px-2 py-1 bg-slate-50 rounded border border-slate-200"
                  >
    ...
    <div className={`border rounded p-2 flex items-center gap-2 ${tone}`}>
      ...
      <div className="min-w-0">
        ...
        <div className="text-sm font-bold tabular-nums truncate">
          {value} <span className="text-[10px] font-normal opacity-70">{sub}</span>
        </div>
      </div>
    </div>
  );
