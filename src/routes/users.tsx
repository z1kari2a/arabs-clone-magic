...
function UsersPage() {
  ...
  const changeRole = async (userId: string, newRole: Role) => {
    ...
    if (userId === user?.id && newRole !== "admin")
      return toast.error("لا يمكن إزالة صلاحيتك كمدير");
    setBusy(true);
    try {
  ...
  return (
    <ErpLayout title="المستخدمون" ribbon={<Ribbon actions={actions} />}>
      ...
      {isAdmin && (
        <div
          className={`rounded p-2 text-xs flex items-center gap-2 border ${allowSignup ? "bg-emerald-50 border-emerald-200 text-emerald-800" : "bg-rose-50 border-rose-200 text-rose-800"}`}
        >
          {allowSignup ? <LockKeyholeOpen size={14} /> : <LockKeyhole size={14} />}
          صفحة إنشاء الحسابات (تسجيل الدخول ← إنشاء حساب) حالياً{" "}
          <b>{allowSignup ? "مفعّلة" : "مغلقة"}</b>— يمكن للزوار{" "}
          {allowSignup ? "تقديم طلب حساب جديد" : "تسجيل الدخول فقط، ولا يمكنهم التسجيل الذاتي"}.
        </div>
      )}
      {showAdd && isAdmin && (
        <div className="bg-white border border-slate-300 rounded p-3 grid grid-cols-1 sm:grid-cols-5 gap-2 items-end">
          <div>
            <label className="text-xs text-slate-500">اسم المستخدم</label>
            <input
              value={nu.username}
          ...
          <div>
            ...
            <input
              value={nu.fullName}
              onChange={(e) => setNu({ ...nu, fullName: e.target.value })}
              className="w-full px-2 py-1 border border-slate-300 rounded text-sm"
            />
          </div>
          <div>
          ...
          <div>
            <label className="text-xs text-slate-500">الصلاحية</label>
            <select
              value={nu.role}
          ...
          <div className="flex gap-2">
            <button
              onClick={addUser}
              disabled={busy}
              className="flex-1 px-2 py-1 bg-emerald-600 text-white rounded text-xs disabled:opacity-50"
            >
              إضافة
            </button>
            <button
      ...
      <div className="bg-white border border-slate-300 rounded">
        <div
          className="px-3 py-2 border-b border-slate-300 flex items-center gap-2 font-semibold text-slate-700"
            ...
            <tr key={u.id}>
              ...
              <td className="border border-slate-200 p-0">
                <select
                  ...
                  className="w-full px-2 py-1 bg-transparent outline-none text-center disabled:cursor-not-allowed disabled:opacity-70"
                >
                  <option value="admin">مدير</option>
                  <option value="user">مستخدم</option>
                  <option value="viewer">مطالع</option>
              ...
              <td className="border border-slate-200 text-center text-xs">
                {u.pending ? (
                  <span className="inline-block px-2 py-0.5 rounded bg-amber-100 text-amber-800">
                    قيد المراجعة
                  </span>
                ) : (
